...
export class UserTableHandle {
  ...
  /**
   * Access to the `wallet` unique index on the table `user`,
   * which allows point queries on the field of the same name
   * via the [`UserWalletUnique.find`] method.
   *
   * Users are encouraged not to explicitly reference this type,
   * but to directly chain method calls,
   * like `ctx.db.user.wallet().find(...)`.
   *
   * Get a handle on the `wallet` unique index on the table `user`.
   */
  wallet = {
